...
const stationIdRequestUrl = url.parse(
  "http://fahrplan.oebb.at/bin/ajax-getstop.exe/dn"
);
const stationDepaturesUrl = url.parse(
  "http://fahrplan.oebb.at/bin/stboard.exe/dn"
);
const authenticationUrl = url.parse(
  "https://tickets.oebb.at/api/domain/v3/init"
);
const timetableUrl = url.parse(
  "https://tickets.oebb.at/api/hafas/v4/timetable"
);
const newStationsSearchUrl = url.parse(
  "https://tickets.oebb.at/api/hafas/v1/stations"
);
...
//     getRequest(newStationsSearchUrl, { count: 15, name: name })
//   );
// };
...
// exports.getJourneys = function (
//   from,
//   to,
//   add_offers = false,
//   date = datetime.create()
// ) {
//   //TODO remove need for node-datetime instead use milliseconds
...
//                       connection,
//                       offer: offers.find(
//                         (offer) => offer.connectionId == connection.id
//                       ),
//                     })),
//                   });
//                 })
...
// findPrices = (ids) => (authentication) => {
//   //Very important has to be same authentication as used for journeys, therefore add directly to get journeys
...
exports.getDelayed = function (stationInfo) {
  ...
  if (journey && journey.length > 0) {
    journey.forEach(function (transport) {
      if (transport.rt) {
...
function generateUserId() {
  return `anonym-${random("A0", 8)}-${random("A0", 4)}-${random(
    "A0",
    2
  )}`.toLowerCase();
}
